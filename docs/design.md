...
```yaml
github:
  mode: auto    # public | unrestricted | scoped | auto (default)
  repos:
...
In `scoped` mode the proxy enforces a writes-only fence across these surfaces, independent of the network allowlist's `permissive` flag:
...
- **REST repo subtree** (`api.github.com/repos/{owner}/{name}/...` and `uploads.github.com/repos/{owner}/{name}/...` with non-read methods): writes to repos outside the listed set return `scope_out_of_scope`. Reads (GET / HEAD / OPTIONS) and requests outside any fenced subtree (`/markdown`, `/search/...`, `/rate_limit`, ...) pass through unchecked — the host's PAT is the outer fence on what the proxy forwards.
...
- **Gist writes** (`api.github.com/gists*` with non-read methods, any non-read on `gist.github.com` including `git-receive-pack`): categorically denied. Gists aren't per-repo and a public gist is the cleanest exfil channel imaginable. 403 `scope_gist_write`.
...
- **User-account writes** (any non-read on `api.github.com/user` and `/user/*`): categorically denied. Closes `POST /user/keys` (persistent SSH backdoor), `POST /user/gpg_keys`, `PATCH /user` (public-bio leak), `POST /user/emails`, and other authenticated-user mutations. 403 `scope_user_account_write`.
- **Org-admin writes** (any non-read on `api.github.com/orgs/{org}/...` not already matched as new-repo creation): categorically denied. Covers team creation, membership grants, webhooks, and other persistence-shaped org operations. 403 `scope_org_admin_write`.
- **Package-registry writes** (any non-read on `*.pkg.github.com`): categorically denied. Path shape varies per package type (npm/maven/docker/nuget); blanket-deny is the safe default until per-repo enforcement is added. 403 `scope_package_write`.

Independent of the fence, `GithubCredentialHandler` strips alternate credential carriers on every scoped-host request as defense-in-depth: the `Cookie` header (web-session auth), `X-GitHub-Token` (legacy alternate header), and `?access_token=` query parameter (deprecated but still recognised by GitHub). agentbox never uses these legitimately, so any presence is either a smuggled credential or noise — drop unconditionally.

`unrestricted` mode bypasses every fence (the explicit "no fence" choice), and `public` mode has no surrogate to write with so the upstream returns 401 without proxy involvement.
