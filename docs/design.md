...
`unrestricted` mode bypasses all three (the explicit "no fence" choice), and `none` mode has no surrogate to write with so the upstream returns 401 without proxy involvement.
